...
```{r}
mnps_suspension_gender_model <- lm(SUSPENSION_PERCENT ~ DATA_VALUE, data = mnps_suspension_by_gender_display_df)

summary(mnps_suspension_gender_model)
```
...
```{r}
t.test(SUSPENSION_PERCENT ~ DATA_VALUE, data = mnps_suspension_by_gender_display_df)
```
...
# Work with MNPS attendance data
```{r}
...
```{r}
# Linear regression: student group is not statistically significant
school_absent_linear = lm(ABSENT_PERCENT ~ STUDENT_GROUP, data = school_absent_df)

school_absent_linear |> summary()
```


```{r}
school_absent_df |> 
    ggplot(aes(x = N_STUDENTS, y = ABSENT_PERCENT, color = STUDENT_GROUP, alpha = 0.5)) +
