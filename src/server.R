...
function(input, output, session) {
    ...
    output$mnps_table <- renderDT({
        subgroup_column_name <- str_to_upper(str_replace_all(input$subgroup_input, " ", "_")) # Format column name for subgroup
        ...
        if (input$area_input == "Metro Nashville Public Schools") {
            ...
            mnps_behavior_df |>
                filter(SUBGROUP == input$subgroup_input) |> # Filter by subgroup input
    ...
    output$mnps_plot <- renderPlot({
        ...
        if (input$area_input == "Metro Nashville Public Schools") {
            mnps_behavior_df |>
                filter(SCHOOL == "*DISTRICT" & SUBGROUP == input$subgroup_input) |> # Filter by district totals and subgroup
